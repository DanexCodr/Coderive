...
(function() {
    ...
    async function loadAllPartials() {
        ...
        const results = await Promise.allSettled([
            ...
            loadPartial('editor-container', 'partials/editor.html')
        ]);
        
        const successCount = results.filter(r => r.status === 'fulfilled' && r.value === true).length;
        console.log(`Loader: loaded ${successCount}/5 partials`);
        ...
        if (successCount === 5) {
            ...
            if (errorDisplay) {
                errorDisplay.innerHTML = `Failed to load ${5 - successCount} partials. Check console.`;
