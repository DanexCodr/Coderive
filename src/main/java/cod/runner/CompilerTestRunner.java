...
public class CompilerTestRunner extends BaseRunner {
    ...
    private static final String NAME = "COMPILER_TEST";
    // --- End Configuration ---
    ...
    public void run(String[] args) throws Exception {
        ...
        DebugSystem.info(NAME + LOG_TAG, "Starting Compiler execution...");
        DebugSystem.info(NAME + LOG_TAG, "Android Base Path: " + ANDROID_PATH);
        DebugSystem.info(NAME + LOG_TAG, "Input file: " + config.inputFilename);
        DebugSystem.info(NAME + LOG_TAG, "Target Mode: " + TARGET_MODE);
        DebugSystem.info(NAME + LOG_TAG, "Target Architecture: " + ARCHITECTURE);

        // 3. Frontend: Parse Source to AST
        // NEW: Create interpreter for unit validation
        Interpreter validationInterpreter = new Interpreter();
        validationInterpreter.setFilePath(config.inputFilename);
        ProgramNode ast = parse(config.inputFilename, validationInterpreter);
        if (ast == null) throw new RuntimeException("Parsing failed.");

        // 4. Middle-end: Compile AST to TAC
        DebugSystem.info(NAME + LOG_TAG, "Generating Three-Address Code...");
        ...
        DebugSystem.info(NAME + LOG_TAG, "TAC generation completed in " + 
    ...
    private String generateNativeOutput(TACProgram program) {
        DebugSystem.info(NAME + LOG_TAG, "Compiling to Native Assembly (" + ARCHITECTURE + ")...");
        ...
        if (profile == null) {
            ...
            DebugSystem.warn(NAME + LOG_TAG, "Architecture " + ARCHITECTURE + " not found, using detected: " + profile.architecture);
        }
        ...
        DebugSystem.info(NAME + LOG_TAG, "Native compilation completed in " + 
    ...
    private void writeOutputToFile(String content, String filename) {
        try {
            DebugSystem.info(NAME + LOG_TAG, "Writing output to: " + filename);
            FileOutputStream fos = new FileOutputStream(filename);
            PrintStream ps = new PrintStream(fos);
            ps.print(content);
            ps.close();
            fos.close();
            DebugSystem.info(NAME + LOG_TAG, "Output successfully written to " + filename);
            DebugSystem.info(NAME + LOG_TAG, "File size: " + content.length() + " bytes");
        } catch (Exception e) {
            DebugSystem.error(NAME + LOG_TAG, "Failed to write output file: " + e.getMessage());
    ...
    public static void main(String[] args) {
        try {
            new CompilerTestRunner().run(args);
        } catch (Exception e) {
            ...
            System.err.println("\nUsage hints:");
            System.err.println("  CompilerTestRunner [options]");
