...
public class LiteralRegistry {
    ...
    private void registerBuiltInHandlers() {
        ...
        define("unicodeEscape",
            new MethodHandler() {
                @Override
                public Object handle(Object literal, List<Object> arguments, ExecutionContext ctx) {
                    return handleStringUnicodeEscape(literal, arguments);
                }
            },
            String.class
        );

        define("unicodeUnescape",
            new MethodHandler() {
                @Override
                public Object handle(Object literal, List<Object> arguments, ExecutionContext ctx) {
                    return handleStringUnicodeUnescape(literal, arguments);
                }
            },
            String.class
        );
    ...
    private Object handleStringUnicodeEscape(Object literal, List<Object> arguments) {
        requireArgCount("unicodeEscape", arguments, 0);
        String target = requireStringTarget(literal, "unicodeEscape");
        StringBuilder out = new StringBuilder(target.length());
        for (int i = 0; i < target.length(); i++) {
            char ch = target.charAt(i);
            if (Character.isHighSurrogate(ch)) {
                if (i + 1 < target.length() && Character.isLowSurrogate(target.charAt(i + 1))) {
                    char low = target.charAt(i + 1);
                    appendUnicodeEscape(out, ch);
                    appendUnicodeEscape(out, low);
                    i++;
                    continue;
                }
                appendUnicodeEscape(out, ch);
    ...
    private Object handleStringUnicodeUnescape(Object literal, List<Object> arguments) {
        requireArgCount("unicodeUnescape", arguments, 0);
        String target = requireStringTarget(literal, "unicodeUnescape");
        StringBuilder out = new StringBuilder(target.length());
        for (int i = 0; i < target.length(); i++) {
            char ch = target.charAt(i);
            if (ch != '\\') {
            ...
            if (i + 1 >= target.length()) {
                throw new ProgramError("unicodeUnescape found trailing backslash");
            }
            char esc = target.charAt(i + 1);
            if (esc != 'u') {
                out.append(ch);
                out.append(esc);
                i++;
            ...
            if (i + 5 >= target.length()) {
                throw new ProgramError("unicodeUnescape found incomplete unicode escape");
            }
            int first = parseHex4(target, i + 2);
            i += 5;

            if (first >= 0xD800 && first <= 0xDBFF) {
                if (i + 6 >= target.length() || target.charAt(i + 1) != '\\' || target.charAt(i + 2) != 'u') {
                    throw new ProgramError("unicodeUnescape missing low surrogate after high surrogate");
                }
                int low = parseHex4(target, i + 3);
                if (low < 0xDC00 || low > 0xDFFF) {
                    throw new ProgramError("unicodeUnescape invalid low surrogate");
                }
                int codePoint = Character.toCodePoint((char) first, (char) low);
                out.append(Character.toChars(codePoint));
                i += 6;
                continue;
            }
            if (first >= 0xDC00 && first <= 0xDFFF) {
                throw new ProgramError("unicodeUnescape unexpected low surrogate");
            }
    ...
    private int parseHex4(String text, int start) {
        int value = 0;
        for (int i = 0; i < 4; i++) {
            int digit = Character.digit(text.charAt(start + i), 16);
            if (digit < 0) {
                throw new ProgramError("unicodeUnescape invalid hex digit in unicode escape");
            }
            value = (value << 4) | digit;
        }
        return value;
    ...
    private void appendUnicodeEscape(StringBuilder out, char ch) {
        out.append("\\u");
