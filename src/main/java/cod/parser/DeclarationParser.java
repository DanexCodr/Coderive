...
public class DeclarationParser extends BaseParser {
    ...
    consume(LBRACE);
    while (!match(RBRACE)) {
// Check fields before methods since both can start with visibility modifiers
        if (isFieldDeclaration()) {
            ...
            // Automatic position sharing - no manual sync needed!
    ...
    // Parse parameters with new syntax: name: type
    consume(LPAREN);
    if (!match(RPAREN)) {
    ...
    do {
        ...
        if (firstSlot) {
            ...
            if (isNamedMode) {
                if (currentToken().type != ID) {
                    throw new ParseError("Mixed slot declaration styles not allowed. Expected name for slot.");
                }
                name = consume(ID).text;
                consume(COLON);
                type = parseTypeReference();
...
private boolean isMethodDeclaration() {
    ...
    // 2. Check for method ID name
    Token nameToken = lookaheadFrom(0, offset);
    if (nameToken == null || nameToken.type != ID) return false;
